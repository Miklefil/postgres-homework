...
--Удаляем товары, снятые с продажи из products
DELETE FROM products WHERE discontinued=1;

--Удаляем из order_details заказы, где нет товаров в наличии
DELETE FROM order_details
WHERE product_id NOT IN (SELECT product_id FROM products);

--Удаляем из orders заказы, где нет товаров в наличии
DELETE FROM orders
